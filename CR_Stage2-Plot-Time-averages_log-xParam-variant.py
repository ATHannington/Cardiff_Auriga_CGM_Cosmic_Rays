...
xlimDict = {
    "R": {"xmin": 1.0, "xmax": 100}, #{"xmin": CRPARAMSMASTER["Rinner"], "xmax": CRPARAMSMASTER["Router"]},
...
if __name__ == "__main__":
    ...
    for halo, allSimsDict in CRSELECTEDHALOES.items():
        ...
        styleDict = apt.get_linestyles_and_colours(selectKeysList,colourmapMain="plasma",colourGroupBy=["no_CRs", "with_CRs_no_Alfven", "with_CRs"],linestyleGroupBy=["standard","high"],lastColourOffset=0.0)
        ...
        for sKey, data in statsOut.items():
            if sKey not in tmpkeepPercentiles:
                dataCopy = copy.deepcopy(data)
                for key,dd in data.items():
                    if key not in tmpkeepPercentiles:
                        for kk, value in dd.items():
                            splitkk = kk.split("_")
                            perc = splitkk[-1]
            ...
            for sKey, data in statsOut.items():
                ...
                for key,dd in data.items():
                    ...
                    for kk,vv in dd.items():
                        splitkk = kk.split("_")
                        newkk = "_".join(splitkk[:-1])
                        tmp2[newkk] = vv
                    
                    tmp3 = cr.cr_calculate_statistics(
                        dataDict=tmp2,
                        CRPARAMS=CRPARAMS,
                    ...
                    dataCopy[key].update(copy.deepcopy(tmp3))
                statsOut[sKey].update(dataCopy)
        ...
        standardResOnlyLegendLabels = {
            (f"standard",f"no_CRs") : f"no_CRs",
            (f"standard",f"with_CRs") : f"with_CRs",
            (f"standard",f"with_CRs_no_Alfven") : f"with_CRs_no_Alfven",
        }
